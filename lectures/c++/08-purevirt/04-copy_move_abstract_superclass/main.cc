...
int main() {
  ...
  Text *pt3 = &t3;
  *pt3 = *pt1;
  printTextBook(t3, "Book 3 (copied by Text pointer)");
  cout << "(note that the partial assignment has been fixed)" << endl;
  ...
  *pm4 = std::move(*pm3); // Force move assignment. Data will be stolen!
  ...
  printTextBook(tm4, "Book 4 (swaped with Book 3 by Text pointer)");
  cout << "(note that the partial assignment has been fixed)" << endl;
  
  // However, if you try to uncomment the lines below, this code will not compile.
  // This is because AbstractBook::operator= is protected, so it cannot be called here.
  /*
  AbstractBook *pb1 = &t1;
  AbstractBook *pb3 = &t3;
  *pb3 = *pb1;
